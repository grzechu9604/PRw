...
#define seqTag "Seq"
#define parTag "Par"
#define messagePattern "%s: Czas wykonania programu: %8.4f sec (%6.4f sec rozdzielczosc pomiaru) R = %d"
...
static const int ROWS = Ns;     // liczba wierszy macierzy
...
void print_elapsed_time(string title, double time, int i, ofstream &file, int r)
{
	...
	char buff[500];
	sprintf(buff, messagePattern, title.c_str(), time, resolution, r);
	string message(buff);
	file << message << endl;
	cout << message << endl;
}
...
int main(int argc, char* argv[])
{
	...
	sequentially_multiply_matrices_IKJ_6_fors(Rbs);
	stop = (double)clock() / CLK_TCK;
	print_elapsed_time(seqTag, stop - start, 0, file, Rbs);

	initialize_matricesZ();
	start = (double)clock() / CLK_TCK;
	parallel_multiply_matrices_IKJ_6_fors(Rbs);
	stop = (double)clock() / CLK_TCK;
	print_elapsed_time(parTag, stop - start, 0, file, Rbs);

	initialize_matrices();
	start = (double)clock() / CLK_TCK;
	sequentially_multiply_matrices_IKJ_6_fors(Rps);
	stop = (double)clock() / CLK_TCK;
	print_elapsed_time(seqTag, stop - start, 0, file, Rps);

	initialize_matricesZ();
	start = (double)clock() / CLK_TCK;
	parallel_multiply_matrices_IKJ_6_fors(Rps);
	stop = (double)clock() / CLK_TCK;
	print_elapsed_time(parTag, stop - start, 0, file, Rps);

	initialize_matrices();
	start = (double)clock() / CLK_TCK;
	sequentially_multiply_matrices_IKJ_6_fors(Rcps);
	stop = (double)clock() / CLK_TCK;
	print_elapsed_time(seqTag, stop - start, 0, file, Rcps);

	initialize_matricesZ();
	start = (double)clock() / CLK_TCK;
	parallel_multiply_matrices_IKJ_6_fors(Rcps);
	stop = (double)clock() / CLK_TCK;
	print_elapsed_time(parTag, stop - start, 0, file, Rcps);
